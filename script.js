...
function battle(USSAssembly, alienShip) {
  while (USSAssembly.hull > 0 && alienShip.hull > 0) {
    if (Math.random() < USSAssembly.accuracy) {
      ...
      displayOutputInAlert(`Successful shot Now alien life is ${alienShip.hull}`) // Attack was successful
    } else {
      displayOutputInAlert(`You missed, try again`) // Attack missed
    }
    ...
    if (USSAssembly.hull > 0 && alienShip.hull > 0) {
      if (Math.random() < alienShip.accuracy) {
        ...
        displayOutputInAlert("Alien attacked you")
        displayOutputInAlert(`Now USS Assembly's life is ${USSAssembly.hull}`)
      } else {
        displayOutputInAlert("Alien tried to attack you")
      }
    }
  }
}
...
function battleNext(USSAssembly, alienShips) {
  // while loop to iterate through alienShips
  while (currentAlienIndex < alienShips.length) {
    ...
    displayOutputInAlert(`Great Job Hero! Alien Ship is destroyed!`)
    currentAlienIndex++
  }

  // Check if all alien ships are destroyed
  if (currentAlienIndex >= alienShips.length) {
    displayOutputInAlert("Congratulations! All the alien ships are destroyed")
  }
}
...
function displayOutputInAlert(message) {
  alert(message);
}
